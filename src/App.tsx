...
type View = 'overview' | 'journey' | 'history' | 'hr' | 'admin';
    ...
    <div className="workspace">
      <header className="topbar">
        <button className="menu-button" onClick={() => setMobileMenu(!mobileMenu)} aria-label="Меню">{mobileMenu ? <X size={22} /> : <Menu size={22} />}</button>
         <div className="breadcrumb">Рабочее пространство <span>/</span> <strong>{view === 'admin' ? 'Администрирование' : view === 'hr' ? 'HR аналитика' : view === 'journey' ? 'Моя траектория' : view === 'history' ? 'История активностей' : 'Обзор'}</strong></div>
