...
import {
  ArrowRight, BookOpen, BriefcaseBusiness, CalendarDays, Check, CheckCircle2,
  ChevronDown, Clock3, Compass, Download, FileUp, Flame, GraduationCap,
  LayoutDashboard, LockKeyhole, Menu, Search, ShieldCheck, Sparkles, Target,
  TrendingUp, UploadCloud, Users, X,
} from 'lucide-react';

type View = 'overview' | 'journey' | 'history' | 'hr';
...
type RecommendationResponse = { source: 'openai' | 'rules'; recommendations: Recommendation[]; emptyReason: string | null };
type HRData = {
...
function App() {
  ...
  const [hrCode, setHrCode] = useState(sessionStorage.getItem('hrCode') || '');
  ...
  useEffect(() => {
    api<{ employees: EmployeeListItem[]; asOf: string }>('/api/bootstrap')
      .then(data => {
        ...
        setAsOf(data.asOf);
        setSelectedId(data.employees.find(item => item.employee_id === 'E0028')?.employee_id || data.employees[0]?.employee_id || '');
  ...
  useEffect(() => {
    if (view === 'hr' && hrCode) loadHR(hrCode);
  }, [view, hrCode]);

  function loadHR(code: string) {
    api<HRData>('/api/hr/dashboard', { headers: { 'x-hr-code': code } })
      .then(data => { setHr(data); setHrError(''); })
      .catch(error => { setHr(null); setHrError(error.message); sessionStorage.removeItem('hrCode'); setHrCode(''); });
  }

  async function unlockHR(event: React.FormEvent) {
    event.preventDefault();
    try {
      await api('/api/hr/verify', { method: 'POST', headers: { 'x-hr-code': hrInput } });
      sessionStorage.setItem('hrCode', hrInput);
      setHrCode(hrInput);
      setHrInput('');
      setHrError('');
    } catch (error) { setHrError((error as Error).message); }
  }
  ...
  async function importData(event: React.FormEvent) {
    ...
    try {
      const result = await api<{ employees: number; history: number }>('/api/hr/import', {
        method: 'POST', headers: { 'x-hr-code': hrCode }, body: form,
      });
  ...
  return <div className="shell">
    <aside className={`sidebar ${mobileMenu ? 'sidebar-open' : ''}`}>
      ...
      <div className="sidebar-label team-label">КОМАНДА</div>
      <nav className="nav-list"><button className={view === 'hr' ? 'active' : ''} onClick={() => changeView('hr')}><Users size={19} /> HR аналитика <span className="nav-indicator" /></button></nav>
      <div className="sidebar-bottom">
        <div className="mini-orbit"><Sparkles size={20} /><span>Каждый шаг<br />имеет значение.</span></div>
        <div className="sidebar-profile"><div className="avatar small">{current ? initials(current.full_name) : 'CQ'}</div><div><strong>{current?.full_name || 'Career Quest'}</strong><small>{current?.role || 'Навигатор развития'}</small></div></div>
      </div>
    </aside>
    ...
    <div className="workspace">
      <header className="topbar">
        ...
        <div className="breadcrumb">Рабочее пространство <span>/</span> <strong>{view === 'hr' ? 'HR аналитика' : view === 'journey' ? 'Моя траектория' : view === 'history' ? 'История активностей' : 'Обзор'}</strong></div>
        <div className="top-actions"><span className="snapshot"><span className="live-dot" /> Срез данных · {dateLabel(asOf)}</span><div className="avatar top-avatar">{current ? initials(current.full_name) : 'CQ'}</div></div>
      </header>

      <main className="main-content">
        {view !== 'hr' && <div className="employee-switcher-wrap">
          <button className="employee-switcher" onClick={() => setSearchOpen(!searchOpen)}><Search size={17} /><span>{current ? `${current.full_name} · ${current.employee_id}` : 'Выберите сотрудника'}</span><ChevronDown size={17} /></button>
        ...
        {view === 'hr' ? (!hrCode ? <section className="unlock-card"><div className="unlock-icon"><LockKeyhole size={26} /></div><span className="eyebrow">ДОСТУП HR</span><h1>Аналитика команды</h1><p>Введите код доступа HR, чтобы увидеть агрегированные данные и загрузить проверочные профили.</p><form onSubmit={unlockHR}><input type="password" placeholder="Код доступа HR" value={hrInput} onChange={event => setHrInput(event.target.value)} /><button className="button primary">Открыть дашборд <ArrowRight size={17} /></button></form>{hrError && <div className="form-error">{hrError}</div>}</section>
          : hr ? <><div className="page-heading"><div><span className="eyebrow">PEOPLE ANALYTICS</span><h1>Пульс развития команды<span className="heading-dot">.</span></h1><p>Где сейчас больше всего разрывов и кому нужен следующий шаг.</p></div><span className="pill violet"><ShieldCheck size={15} /> Доступ HR</span></div>
          ...
          </> : <div className="loading-page">Загружаем HR-аналитику...</div>)
